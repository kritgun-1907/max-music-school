  ...
  import { redisService } from './redis.service'; // Import the redis service
  import { z } from 'zod';
  ...
  this.isInitialized = true;
    ...
    async getStudentByEmail(email: string): Promise<any | null> {
      try {
        ...
        await redisService.set(cacheKey, JSON.stringify(student), 300);
    ...
    async updateStudentData(studentId: string, updates: Partial<any>): Promise<any> {
      try {
        ...
        await redisService.del(`student:email:${updatedData.email}`);
    ...
    public async addStudentRow(rowData: any[]): Promise<void> {
      try {
        ...
        await redisService.del('students:all');
      } catch (error) {
    ...
    async getTeacherByEmail(email: string): Promise<any | null> {
      try {
        ...
        await redisService.set(cacheKey, JSON.stringify(teacher), 300);
    ...
    async getTeacherBatches(teacherName: string): Promise<any[]> {
      try {
        ...
        const teacherStudents = rows.filter((row, index) => {
          if (index === 0) return false;
          return row[17] === teacherName;
        });

        const batchesMap = new Map();
        
        teacherStudents.forEach(row => {
          const batchKey = `${row[4]}-${row[7]}-${row[8]}`;
    ...
    async storeRefreshToken(userId: string, token: string): Promise<void> {
      ...
      await redisService.set(`refresh_token:${userId}`, token, 604800);
    }
